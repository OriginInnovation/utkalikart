...
<head>
    ...
    <title>Indus-Supply | Sub-subcategory</title>
                                    ...
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center"><img
                                                    src="upload/subsubcategory/<?php echo $row['image']; ?>"
                                            ...
                                            <td class=text-center>
                                                ...
                                                <button type="button" name="update1" class="btn btn-primary btn-sm m-2"
                                                    onclick="myfcn5(<?php echo $row['id']; ?>,'<?php echo $row['image']; ?>','<?php echo $row['sub_subcategoryname']; ?>','<?php echo $row['sub_category_id']; ?>','<?php echo $row['category_id']; ?>')"
                ...
                <div class="modal-body">
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="file" required name="image3" class="form-control text-white"
                                accept="image/jpeg, image/jpg, image/png"
                                onchange="document.getElementById('preview3').src = window.URL.createObjectURL(this.files[0])">
                            <img id="preview3" src="img/no_image.png" alt="image3" width="50" height="50" />
                        </div>
...
if (isset($_POST['sub_subcategory_insert'])) {
    ...
    $new_file_name4 = uniqid() . '.' . $file_type;

    $upload_dir = "upload/subsubcategory/";
    ...
    $target_file = $upload_dir . $new_file_name4;
    ...
    if ($result3->num_rows > 0) {
        ...
        // Use sanitized input in the INSERT query
        $sql = "INSERT INTO sub_subcategory (image, sub_subcategoryname, category_id, sub_category_id,  status) VALUES ('$new_file_name4', '$name','$value','$valuee','1')";
...
if (isset($_POST['sub_subcategory_update'])) {
    include 'conn.php';
    $image_name = $_FILES['sub_subcat_image']['name'];
    $id = $_POST["id89"];
    ...
    if ($image_name == NULL) {
        ...
        $image_size = $_FILES['sub_subcat_image']['size'];
        $image_tmp = $_FILES['sub_subcat_image']['tmp_name'];
        $file_type = pathinfo($image_name, PATHINFO_EXTENSION);
        $new_file_name = uniqid() . '.' . $file_type;
        $upload_dir = "upload/subsubcategory/";

        // Retrieve the previous file name from the database
        $sql_previous_image = "SELECT image FROM sub_subcategory WHERE id='$id'";
        ...
        if (move_uploaded_file($image_tmp, $target_file)) {
            ...
            $sql1 = "UPDATE sub_subcategory SET image='$new_file_name', sub_subcategoryname='$sub_subcategory_name', sub_category_id='$sub_category_name', category_id='$category_name' WHERE id='$id'";
            if ($conn->query($sql1) == true) {
                echo "<script>
                    $(document).ready(function(){
                    toastr.success('Form submitted successfully');
